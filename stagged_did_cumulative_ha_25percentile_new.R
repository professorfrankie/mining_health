...
process_treatment_data <- function(path) {
  ...
  # 1. Compute global p25 cutoff (mine basin–year level)
  mining_p25_mine <- data_raw %>%
    filter(status == "mine") %>%
    group_by(mine_basin, year) %>%
    summarise(
      cumulative_mining = first(cumulative_mining),
      .groups = "drop"
    ) %>%
    summarise(
      p25 = quantile(cumulative_mining, 0.25, na.rm = TRUE)
    ) %>%
    pull(p25)
  ...
  data_processed <- data_raw %>% 
    ...
    mutate(
      mining_p25_mine = mining_p25_mine,
...
compute_p25 <- function(data, mining_var) {
  data %>%
    filter(status == "mine") %>%
    ...
    summarise(
      p25 = quantile(mining, 0.25, na.rm = TRUE)
    ) %>%
    pull(p25)
}
...
build_treatment <- function(
  ...
  # Compute cutoff ONCE
  p25 <- compute_p25(data_raw, mining_var)
...
treatment_capital_child_mort_2019_art <- build_treatment(
  ...
  mining_var = "cumulative_artisanal",
...
treatment_capital_child_mort_2019_art_share <- build_treatment(
  path = "/home/francesca/brazil_mining/mining_health/processed data/treatment_capital_child_mort.gpkg",
  mining_var = "cumulative_artisanal",
  share_var = "cumulative_artisanal_share",
  share_cutoff = 0.5
)
...
treatment_capital_child_mort_2019_ind <- build_treatment(
  path = "/home/francesca/brazil_mining/mining_health/processed data/treatment_capital_child_mort.gpkg",
  mining_var = "cumulative_industrial",
...
treatment_capital_child_mort_2019_ind_share <- build_treatment(
  path = "/home/francesca/brazil_mining/mining_health/processed data/treatment_capital_child_mort.gpkg",
  mining_var = "cumulative_industrial",
  share_var = "cumulative_industrial_share",
  share_cutoff = 0.5
)
