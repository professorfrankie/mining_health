...
treatment_capital_child_mort <- st_read("/home/francesca/brazil_mining/mining_health/processed data/treatment_capital_child_mort.gpkg") %>% 
  ...
  mutate(
    mining_median_mine = median(mining_ha_total[status == "mine"], na.rm = TRUE),
...
treatment_capital_child_mort_2019_art <- st_read("/home/francesca/brazil_mining/mining_health/processed data/treatment_capital_child_mort.gpkg") %>% 
  ...
  ungroup() %>% 
  mutate(
    mining_median_mine = median(mining_ha_artisanal[status == "mine"], na.rm = TRUE),
    crossed = mining_ha_artisanal > mining_median_mine
  ) %>% 
  group_by(muni_id) %>% 
  mutate(
    G = case_when(
...
treatment_capital_child_mort_2019_ind <- st_read("/home/francesca/brazil_mining/mining_health/processed data/treatment_capital_child_mort.gpkg") %>% 
  ...
  ungroup() %>% 
  mutate(
    mining_median_mine = median(mining_ha_industrial[status == "mine"], na.rm = TRUE),
    crossed = mining_ha_industrial > mining_median_mine
  ) %>%
  group_by(muni_id) %>% 
  mutate(
    G = case_when(
